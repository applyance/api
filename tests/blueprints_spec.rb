  ...
  describe "POST #entities/blueprints" do
    context "logged in as full reviewer" do
      ...
      before(:each) do
        ...
        post "/entities/#{spot.entity.id}/blueprints", Oj.dump({ definition_id: definition.id, position: 1, is_required: false }), { "CONTENT_TYPE" => "application/json" }
      end

      it_behaves_like "a created object"
      it_behaves_like "a single blueprint"
      it "returns the right value" do
        expect(spot.entity.blueprints.collect(&:definition_id)).to include(definition.id)
        expect(spot.blueprints.collect(&:definition_id)).not_to include(definition.id)
