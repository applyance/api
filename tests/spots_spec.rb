...
describe Applyance::Spot do
  ...
  shared_examples_for "a single spot" do
    it "returns the information for spot show" do
      expect(json.keys).to contain_exactly('id', 'entity', 'name', 'detail', 'status', 'created_at', 'updated_at')
    end
  end

  shared_examples_for "multiple spots" do
    it "returns the information for spot index" do
      expect(json.first.keys).to contain_exactly('id', 'entity_id', 'name', 'detail', 'status', 'created_at', 'updated_at')
    end
  end
